...
export default function ArchiveSlider() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const { payload: products, error } = await getProducts();
    ...
    >
      {randomProducts.map((item, index) => (
        <SwiperSlide key={index} className="pb-12">
          <HoverCard item={item} id={item.id} />
        </SwiperSlide>
      ))}
    </Swiper>
  );
...
const HoverCard = ({ item, id }) => {
  ...
  return (
    ...
    >
      ...
      <Button
        text="Ver Detalles"
        href={`/${item.product}/${item.category}/${encodeURIComponent(id)}`}
        className="flex items-center justify-center align-middle font-raleway hover:text-gray-500 transition duration-200"
